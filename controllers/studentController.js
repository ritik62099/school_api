...
export const promoteStudent = async (req, res) => {
  ...
  try {
    ...
    const { newClass } = req.body;        // frontend se aayega
    if (!newClass) {
      return res.status(400).json({ message: "New class is required" });
    }

    await session.withTransaction(async () => {
      // 1️⃣ Student ki class + roll reset
      const student = await Student.findByIdAndUpdate(
        id,
        { class: newClass, rollNo: "" },
        { new: true, session }
      );
      ...
      // 2️⃣ Marks reset – pura record hata do (naya session clean start)
      await Marks.deleteOne({ studentId: id }, { session });

      // 3️⃣ Attendance reset – jitne bhi Attendance docs me ye student hai, waha se nikal do
    ...
    res.json({ message: "Student promoted and marks/attendance reset successfully" });
  } catch (err) {
    console.error("Promote Student Error:", err);
    res.status(500).json({ message: err.message || "Error while promoting student" });
  } finally {
    session.endSession();
  }
};
